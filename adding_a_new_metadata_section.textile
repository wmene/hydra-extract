...
Add an accordion section into vendor/plugins/hydrangea_datasets/views/_edit.html.haml and set it to render a partial called "edit_license"

h2. Create a new partial for the new Accordion Section

Create a partial at vendor/plugins/hydrangea_datasets/views/_edit_license.html.haml

h2. Fill in template for the accordion section

refer to vendor/plugins/hydrangea_datasets/views/_edit_description.html.haml and imitate its code

h3. Create the form tag

h3. Put Fields into the Template
...
Log in as an archivist (ie. archivist1).  If you haven't already created a user, create a user called archivist1 by clicking on "create an account" on the login page.

Click the dropdown on the "add an Article" button and select "add a Dataset"

You should see a Dataset with the accordion section you've just added.

h2. Set up options for use in the select tags
